...
s21_decimal s21_add(s21_decimal src1, s21_decimal src2) {
    ...
    if (res.value_type == 0) {
        ...
        if (res.value_type == 1) {
            ...
            if (src1.value_type == 1 && src2.value_type == 1) res.value_type = s21_INFINITY;
            else
            res.value_type = s21_NEGATIVE_INFINITY;
        } else if (res.value_type == 4) {
            if (src1.value_type == 2 && src2.value_type == 2) res.value_type = s21_NEGATIVE_INFINITY;
            else
...
s21_decimal s21_sub(s21_decimal src1, s21_decimal src2) {
    ...
    if (res.value_type == 0) {
        ...
        if (res.value_type == 1) {
            if (src1.value_type == 0) res.value_type = s21_NEGATIVE_INFINITY;
            else
                res.value_type = s21_INFINITY;
        } else if (res.value_type == 2) {
            if (src1.value_type == 1 && src2.value_type == 1) {
                ...
                if (src1.value_type == 0) res.value_type = s21_INFINITY;
                else
                    res.value_type = s21_NEGATIVE_INFINITY;
            }
        } else if (res.value_type == 3) {
            res.value_type = s21_NAN;
            if (src1.value_type == 1) res.value_type = s21_INFINITY;
            else
                res.value_type = s21_NEGATIVE_INFINITY;
        } else if (res.value_type == 4) {
            res.value_type = s21_NAN;
...
s21_decimal s21_mul(s21_decimal src1, s21_decimal src2) {
    ...
    if (val == 0) {
        ...
        zero_sDecimal(&tmp1);
        zero_sDecimal(&tmp2);
        tmp1.bits[6] = src1.bits[3];    tmp2.bits[6] = src2.bits[3];
        tmp1.bits[2] = src1.bits[2];    tmp2.bits[2] = src2.bits[2];
        tmp1.bits[1] = src1.bits[1];    tmp2.bits[1] = src2.bits[1];
        tmp1.bits[0] = src1.bits[0];    tmp2.bits[0] = src2.bits[0];
        ...
        if ((src1.value_type == 1 && src2.value_type == 1) ||
            ...
            (src1.value_type == 0 && elder_bit(src1) != -1 &&
                check_bit(src1, 127) == 0  && src2.value_type == 1)) result.value_type = 1;
            ...
            (src1.value_type == 1 && src2.value_type == 2)) result.value_type = 2;
        else
            result.value_type = 3;
    }
    return result;
...
s21_decimal s21_div(s21_decimal src1, s21_decimal src2) {
    ...
    if (val == 0 && elder_bit(src2) != -1) {
        ...
        if ((src1.value_type == (1 || 2) && src1.value_type == (1 || 2)) ||
            ...
            (elder_bit(src1) == -1 && elder_bit(src2) == -1)) res.value_type = 3;
        else if
            ((src1.value_type == 0 && check_bit(src1, 127) == 0 && src2.value_type == 2) ||
            (src1.value_type == 0 && check_bit(src1, 127) == 1 && src2.value_type == 1) ||
            (src1.value_type == 0 && check_bit(src1, 127) == 1 && elder_bit(src2) == -1 &&
                check_bit(src2, 127) == 0 && src2.value_type == 0) ||
            (src1.value_type == 0 && check_bit(src1, 127) == 0 && elder_bit(src2) == -1 &&
                check_bit(src2, 127) == 1 && src2.value_type == 0)) res.value_type = 2;
        else
            res.value_type = 1;
    }
    return res;
...
s21_decimal s21_mod(s21_decimal src1, s21_decimal src2) {
    ...
    if (s21_is_greater(src1, src2) == TRUE && val == 0) {
        ...
        if (s21_is_greater(src1, src2) == FALSE && val == 0) {
            res = src1;
            res.bits[3] |= sgn;
        } else {
            zero_decimal(&res);
